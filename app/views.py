...
from app.data_types import FilterViewContext, ViewType, ViewContext
...
def view_stores(view_type: str):
    ...
    view_context = ViewContext(view_type=view_type_enum.value, stores=get_stores(), bbox=bbox)

    if view_type_enum is ViewType.API:
        return view_context.stores.to_dict()
    return render_template(f"{view_type_enum.value}.html", **view_context.__dict__)
